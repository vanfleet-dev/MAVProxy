#!/usr/bin/env python3
'''
swarm2 module - Multi-vehicle guided control

Commands:
- swarm2 status               : Show all detected vehicles and their configured altitudes
- swarm2 alt <sysid> <alt>    : Set altitude for a vehicle (e.g., "swarm2 alt 1 50")
- swarm2 guided               : Send all configured vehicles to map click location
- swarm2 clear                : Clear all configured altitudes
'''
...
class Swarm2Module(mp_module.MPModule):
    def __init__(self, mpstate):
        super(Swarm2Module, self).__init__(mpstate, "swarm2", "swarm2 module", multi_vehicle=True, public=True)
        ...
        self.add_command('swarm2', self.cmd_swarm2, "swarm2 multi-vehicle control", [
            "status",
            "alt",
            "guided",
            "clear"
    ...
    def cmd_swarm2(self, args):
        '''handle swarm2 commands'''
        if len(args) == 0:
            print("Usage: swarm2 <status|alt <sysid> <alt>|guided|clear>")
            return
        ...
        if cmd == "status":
            ...
            if len(args) != 3:
                print("Usage: swarm2 alt <sysid> <altitude>")
                return
            try:
                sysid = int(args[1])
                ...
                print("Invalid sysid or altitude. Usage: swarm2 alt <sysid> <altitude>")
        elif cmd == "guided":
    ...
    def send_guided_commands(self):
        ...
        if not self.vehicle_altitudes:
            print("No vehicles configured. Use 'swarm2 alt <sysid> <altitude>' first.")
            return
